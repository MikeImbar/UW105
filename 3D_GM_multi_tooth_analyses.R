...
SurfaceModel <- file2mesh("O:/Research/DentalTissues/FOSSIL/UW_105/X0003976_000_001_UW_105-681a_URM/FILT1_1/UW105_681a-files/UW105_681ab_combined_EDJ_patched.ply")
...
# nearest neighbour based on PCs distances
dist_mat <- as.matrix(dist(Proc$PCscores[,1:3]))
rownames(dist_mat)
PC_distances <- sort(dist_mat[which(rownames(dist_mat) == "UW105-1007_ULP4"),])

# only_nearest_neighbour
# Plot PCA PC1 V 2
...
# texts3d(cbind(Proc$PCscores[,1],Proc$PCscores[,2],Proc$PCscores[,3]),texts = rownames(Proc$PCscores),font=1,adj=1,cex=0.8)

for(a in setdiff(seq_along(levels(gp)), c(3))){
  sub<-gp==levels(gp)[a]
...
save_location <- "D:/3D_GM/Projects/UW105/Results/Taxonomic_assessment/UW105-750_and_1007"
setwd(save_location)
widget <- rglwidget(webgl = TRUE, snapshot = FALSE, width = 1770, height = 1000)
htmlwidgets::saveWidget(widget, file.path(save_location, "3D_PCA_clean_updated_as_UP3s.html"))

# Save 3D PCA as svg
rgl.postscript("3D_PCA_clean_updated_as_UP3s.svg",fmt="svg")
...
rownames(dist_mat)
sort(dist_mat[which(rownames(dist_mat) == "UW_105_002A_LLM2"),])

# compare specimens
as.matrix(classifier$Name)
#specimens<-c(1,17); cbind(as.character(classifier$Name[c(specimens)]),farbe[c(1:length(specimens))])
specimens<-c(58,47); cbind(as.character(classifier$Name[c(specimens)]),farbe[c(1:length(specimens))])
show_landmarks = F
...
for(i in 1:length(specimens)){
  lapply(seq(curves), function(x){
    lines3d(Proc$rotated[curves[[x]],,specimens[i]], col=farbe[gp][specimens[i]],lwd=2)
    if(show_landmarks){
      points3d(Proc$rotated[curves[[x]],,specimens[i]],col=farbe[gp][specimens[i]], size = 7)
    }
    spheres3d(Proc$rotated[fix,,specimens[i]],radius=0.003, col=farbe[gp][specimens[i]])
  })
}
legend3d("topleft", cbind(as.character(classifier$Name[c(specimens)])), pch = 16, col = farbe[gp][specimens], cex = 3)


## compare group means
# optional modification of the groups
gp_char <- as.character(gp)
gp_char[46] <- "UW_105_240_LLP"
gp_char[45] <- "UW105_811_LLP4"
gp <- as.factor(gp_char)
farbe <- c(farbe, "black", "black")
...
gp_char <- as.character(gp)
gp_char[58] <- "UW_105_002A"
gp_char[57] <- "UW105-825_LRM2"
gp <- as.factor(gp_char)
# farbe[4] <- "black"
farbe <- c(farbe[!is.na(farbe)], "black")

dist_and_alpha_list<- nearest_neighbours_with_alpha(Proc$rotated, 7, 0.1 , 0.8)
  ...
  lapply(seq(curves), function(x){
    ...
    if(sum(sub) > 1 & show_specimens){
      for(s in 1:dim(groups_specs)[[3]]){
        lines3d(groups_specs[curves[[x]],,s],col=farbe[groups[i]],lwd=1.2, alpha = 0.4)
      }
    }
    spheres3d(group_mshape[fix,],radius=0.003, col=farbe[groups[i]])
  })
...
if(!is.null(curves$CEJ)){
  CEJ_curve_lms <- unique(curves$CEJ)
  manual_CEJ_csize <- apply(Landmarks_slid[CEJ_curve_lms, , ], 3, cSize)
} else {
  print("No CEJ curve")
}

# for(m in 1:dim(Landmarks_slid[unique(curves[length(curves)][[1]]),,])[3]){
#   points3d(Landmarks_slid[unique(curves[length(curves)][[1]]),,m])
#   lines3d(Landmarks_slid[unique(curves[length(curves)][[1]]),,m])
# }

# SizeFromProc <- Proc$size

data.cs <- data.frame(CSize = log(manual_CEJ_csize), Group = gp, Spec = rownames(Proc$PCscores))
...
for(i in start_PC:end_PC){
  print(i)
  if(DROPLMs){
    CVA_res<-CVA(dataarray=Proc$PCscores_drop[classified,1:i],groups=gp_class,weighting = TRUE, cv = TRUE, prior=rep(1/groupnum_c,groupnum_c), plot = F)
  } else {
    CVA_res<-CVA(dataarray=Proc$PCscores[classified,1:i],groups=gp_class,weighting = TRUE, cv = TRUE, prior=rep(1/groupnum_c,groupnum_c), plot = F)
  }
  ...
  posterior_probabilities[[paste0(i, "_PCs")]] <- test$posterior
  rownames(posterior_probabilities[[paste0(i, "_PCs")]]) <- rownames(coords_to_project)
  ...
  if(ncol(CVA_res$CVscores) == 1){
    CV_data <- data.frame(CV = c(CVA_res$CVscores, projected), Group = gp, Spec = rownames(Proc$PCscores))
    p <- CV_data %>%
...
CVA_results <- list(
  ...
  PosteriorProbs = posterior_probabilities
)
...
sink("D:/3D_GM/Projects/UW105/Results/Taxonomic_assessment/UW105_658/CVA_results.txt")
print(CVA_results)
sink()
...
plot(cbind(Proc$PCscores_drop[,1],Proc$PCscores_drop[,2]),type="n",asp=1,cex=1,
     xlab=paste0("PC 1 ", "(", round(Proc$Variance[1,2], 2), "%", ")"),
     ylab=paste0("PC 2 ", "(", round(Proc$Variance[2,2], 2), "%", ")"))
for(a in setdiff(seq_along(levels(gp)), c(8))){
  sub<-gp==levels(gp)[a]
  tr <- NULL
...
plot3d(cbind(Proc$PCscores_drop[,1],Proc$PCscores_drop[,2],Proc$PCscores_drop[,3]),type="p",lwd=12,box=FALSE,size=10,col=farbe[gp],aspect =T,axes=T,
       ...
       ylab = paste("PC2 (", round(Proc$Variance[2,2],1), "%)", sep = ""),
       zlab = paste("PC3 (", round(Proc$Variance[3,2],1), "%)", sep = ""))
# texts3d(cbind(Proc$PCscores_drop[,1],Proc$PCscores_drop[,2],Proc$PCscores_drop[,3]),texts = classifier$Name,font=1,adj=1,cex=0.8)
legend3d("topleft", levels(gp), pch = 16, col = farbe, cex = 3)
...
#Save 3D PCA plot as HTML file
save_location <- "D:/3D_GM/Projects/UW105/Results/Taxonomic_assessment/UW105_811"
setwd(save_location)
widget <- rglwidget(webgl = TRUE, snapshot = FALSE, width = 1770, height = 1000)
...
rownames(dist_mat)
sort(dist_mat[which(rownames(dist_mat) == "UW105_681_URM"),])

# compare specimens
as.matrix(classifier$Name)
#specimens<-c(1,17); cbind(as.character(classifier$Name[c(specimens)]),farbe[c(1:length(specimens))])
specimens<-c(42,18); cbind(as.character(classifier$Name[c(specimens)]),farbe[c(1:length(specimens))])
...
for(i in 1:length(specimens)){
  lapply(seq(curves), function(x){
    lines3d(Proc$rotated[curves_drop_orgn[[x]],,specimens[i]], col=farbe[gp][specimens[i]],lwd=2)
    if(show_landmarks){
      points3d(Proc$rotated[curves_drop_orgn[[x]],,specimens[i]],col=farbe[gp][specimens[i]], size = 7)
    }
    match_fixed <- curves_drop_orgn[[x]] %in% fix 
    if(any(match_fixed)){
      spheres3d(Proc$rotated[curves_drop_orgn[[x]][match_fixed],,specimens[i]],radius=0.003, col=farbe[gp][specimens[i]])
    }
  })
}
legend3d("topleft", cbind(as.character(classifier$Name[c(specimens)])), pch = 16, col = farbe[gp][specimens], cex = 3)
